...
- **Lottery:** [`/lottery`](<./src/app/(app)/lottery/page.tsx>) digitizes low-census staffing decisions with a fixed full-time lottery list, date/shift volunteer requests, keep-to-work recommendation previews, manager overrides, and live schedule application for `on call` / `cancelled`
...
- `/lottery` uses those same live assignment statuses to apply low-census recommendations and to repair request/history state when `on call` / `cancelled` is later undone.
