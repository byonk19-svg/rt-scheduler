...
- **Lottery workflow:** `/lottery` is now a shared authenticated route for low-census staffing decisions. It reads live published schedule assignments by date/shift, keeps a fixed full-time lottery order per shift, tracks date-based volunteer requests with restorable suppression, previews keep-to-work recommendations, lets leads/managers apply results to the live schedule, and records repairable lottery history tied to real `on_call` / `cancelled` status changes.
