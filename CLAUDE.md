...
- **Lottery workflow:** `/lottery` is now the manager/lead low-census workspace. It loads live published-shift data, supports request sign-up/removal, fixed lottery list maintenance, preview/apply of reductions, and therapist history review. Targeted verification now includes `src/app/api/lottery/workflow-routes.test.ts`, the recommendation/status-reconciliation unit tests under `src/lib/lottery/`, and the manager browser flow in `e2e/lottery-workflow.spec.ts`.
...
- Current branch (`claude/general-session-vZn3h`) now also carries the lottery workflow moved in from the local prototype snapshot so the active checkout contains the `/lottery` page, lottery API routes, and the focused request/list/apply/history verification.
