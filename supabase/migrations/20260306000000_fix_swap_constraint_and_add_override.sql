-- Fix the swap approval unique constraint bug.
-- The trigger swaps shifts via two sequential UPDATE statements. The intermediate
-- state (both user_ids on the same date) violates the non-deferrable unique index.
-- Converting to a named DEFERRABLE constraint lets the trigger defer the check
-- within its transaction, while keeping INITIALLY IMMEDIATE everywhere else.

drop index if exists public.shifts_unique_cycle_user_date_idx;

alter table public.shifts
  add constraint shifts_unique_cycle_user_date
    unique (cycle_id, user_id, date)
    deferrable initially immediate;

-- Add manager_override support to shift_posts so managers can force-approve
-- requests that violate soft scheduling constraints (e.g. lead coverage gap).
...
--   1. Defer the unique constraint during swap (fixes the transient duplicate bug).
--   2. Skip lead-coverage-gap / lead-eligibility checks when manager_override = true.
