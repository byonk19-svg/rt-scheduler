-- Reassert the active operational-entry guard after the team swap partner
-- workflow migration accidentally reused an older cleared_at check.
...
declare
  ...
  recipient_has_matching_shift boolean;
  ...
  if p_claimed_by is not null then
    ...
    if p_type = 'swap' then
      select exists (
        ...
        into recipient_has_matching_shift;

      if recipient_has_matching_shift is not true then
        raise exception 'Swap partner must already be scheduled on the same date and shift type.';
      end if;
    elsif p_type = 'pickup' then
      if coalesce(p_visibility, 'team') = 'team' then
        raise exception 'Team pickup requests cannot preselect a recipient.';
      end if;
