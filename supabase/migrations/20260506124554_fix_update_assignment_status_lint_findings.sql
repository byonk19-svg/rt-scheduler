...
declare
  ...
  v_previous_status public.assignment_status := 'scheduled'::public.assignment_status;
  v_previous_note text;
  v_previous_left_early_time time;
  v_next_status public.assignment_status := 'scheduled'::public.assignment_status;
  v_next_note text;
...
begin
  ...
  if v_active_entry_id is null then
    v_previous_status := 'scheduled'::public.assignment_status;
    v_previous_note := null;
    v_previous_left_early_time := null;
  end if;

  if p_status = 'scheduled' then
    v_next_status := 'scheduled'::public.assignment_status;
    v_next_note := null;
    ...
    if v_active_entry_id is not null then
      ...
      where public.shift_operational_entries.id = v_active_entry_id;
