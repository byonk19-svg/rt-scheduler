...
- **Neutrals:** White page background (`hsl(0 0% 100%)`), near-white cards (`hsl(38 14% 99%)`), muted inner panels (`#f7f5f1`), cool-warm slate text (`~hsl(220 25% 12%)` foreground), muted lines at `~hsl(32 14% 86%)`. The earlier warm paper field (`#f0ebe1`) was retired — cards and inner panels keep a fractional warm tint for depth without the page reading as tan.
- **Semantic:**
...
- **Background warmer paper:** `--background` moved from `hsl(38 18% 96%)` (`#F8F4ED`) → `hsl(38 24% 91%)` (`#f0ebe1`, the design's `--bg-page`). **Superseded 2026-04-28:** `--background` changed to `hsl(0 0% 100%)` (white) — see Decisions Log.
...
### Shipped wiring (2026-04-28)

- **White page background:** `--background` changed from `hsl(38 24% 91%)` (`#f0ebe1`, warm paper) → `hsl(0 0% 100%)` (pure white) in `:root` and the `@media print` forced-light override in `src/app/globals.css`. Applied site-wide through `bg-background` on the AppShell wrapper. `--card` (`hsl(38 14% 99%)`) and `--muted` (`#f7f5f1`) are unchanged — cards and inner panels retain a fractional warm tint for surface hierarchy without the overall page reading as tan. Print output is also white.
