...
function getFeedback(
  ...
  if (success === 'work_pattern_saved') {
    return { message: 'Work pattern saved.', variant: 'success' }
  }
  ...
  if (error === 'save_failed') {
    return { message: 'Could not save the work pattern. Please try again.', variant: 'error' }
  }
...
async function saveManagerRecurringPatternAction(therapistId: string, formData: FormData) {
  ...
  try {
    ...
    redirect(`/team/work-patterns/${therapistId}?error=invalid_pattern`)
  }
  ...
  if (error) {
    console.error('Failed to save manager recurring pattern:', error)
    redirect(`/team/work-patterns/${therapistId}?error=save_failed`)
  }
...
export default async function TeamWorkPatternDetailPage({
  params,
  searchParams,
}: {
  ...
  const { data: profile } = await supabase
    ...
    .eq('id', routeParams.therapistId)
    .in('role', ['therapist', 'lead'])
    .maybeSingle()

  if (!profile) {
    redirect('/team/work-patterns')
  }
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border bg-card px-6 pb-4 pt-5">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Update the recurring pattern for {profile.full_name ?? 'this therapist'}.
          </p>
        </div>
        <Button asChild variant="outline">
          <Link href="/team/work-patterns">Back to work patterns</Link>
        </Button>
      </div>

      <RecurringPatternEditor
        initialPattern={pattern}
        saveAction={saveManagerRecurringPatternAction.bind(null, profile.id)}
      />
    </div>
  )
}
