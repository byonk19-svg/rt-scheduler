...
export default async function StaffSwapHistoryPage({
  ...
  const { data, error } = await supabase
    .from('shift_posts')
    .select(
      'id, status, message, created_at, shift_id, posted_by, claimed_by, shifts!shift_posts_shift_id_fkey(date, shift_type), poster:profiles!shift_posts_posted_by_fkey(full_name), claimer:profiles!shift_posts_claimed_by_fkey(full_name)'
    )
    .or(filter)
  ...
  if (error) {
    console.error('Failed to load shift post history:', error)
  }

  const rows = (data ?? []) as ShiftPostHistoryRow[]
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">
              Posts you created and pickups or swaps you claimed, newest first.
            </p>
          </div>
      ...
      {rows.length === 0 ? (
        <div className="flex flex-col items-center gap-2 rounded-xl border border-border/70 bg-card px-6 py-10 text-center shadow-tw-sm">
          ...
          <p className="max-w-sm text-xs text-muted-foreground">
            When you post or claim a shift on the board, it will show up here.
          </p>
                ...
                {rows.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className="hover:bg-secondary/15">
                      ...
                      <td className="px-4 py-2.5 text-muted-foreground">{partnerName}</td>
                      <td className="px-4 py-2.5">
                        <span
