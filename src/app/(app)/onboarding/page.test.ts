...
describe('onboarding route copy', () => {
  ...
  it('locks the onboarding hub copy and required step labels', () => {
    ...
    expect(setupSource).toContain('getSundayOnOrBeforeKey')
    expect(setupSource).not.toContain('getMondayOnOrBeforeKey')
    expect(setupSource).toContain('Week {weekIndex + 1}')
    expect(setupSource).toContain('Weeks are shown Sunday-Saturday.')
    expect(setupSource).toContain('getPreviewStatusLabel')
    expect(setupSource).not.toContain('Blank')
    expect(setupSource).toContain('Updates as you choose days')
    expect(setupSource).toContain('Preview based on this setup')
    ...
    expect(setupSource).toContain('AlertDialogTrigger asChild')
    expect(setupSource).toContain('Leave setup?')
    expect(setupSource).toContain('Your setup is not saved yet. You can come back later to finish.')
    expect(setupSource).toContain('Keep setting up')
    expect(setupSource).toContain("window.location.assign('/login')")
    expect(setupSource).not.toContain('Exit setup\n        </Link>')
    ...
    expect(setupSource).toContain('Save and view my schedule')
    ...
    expect(actionSource).toContain('hasFixedNeverAvailableConflict')
    ...
    expect(source).toContain("redirect('/schedule?setup=complete')")
  ...
  it('redirects legacy completed setup URLs to the schedule instead of rendering an interstitial', async () => {
    mockOnboardingContext()
    ...
    ).rejects.toThrow('REDIRECT:/schedule?setup=complete')
  })
})
