...
export async function POST(request: Request) {
  ...
  const mutation = await updateAssignmentStatusWithLottery({
    ...
    actor: {
      ...
      role: parseRole(actorProfile.role) === 'manager' ? 'manager' : parseRole(actorProfile.role) === 'lead' ? 'lead' : 'therapist',
  ...
  if (!mutation.ok) {
    ...
    return NextResponse.json({ error: mutation.error || 'Could not update assignment status.' }, { status: 500 })
  }
