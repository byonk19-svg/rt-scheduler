...
function Logo() {
  return (
    <div className="flex items-center gap-2.5">
      <div className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-sidebar-accent">
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="flex min-h-screen items-start">
        <aside className={cn(APP_SHELL_SIDEBAR_CLASS, 'w-60 print:hidden')}>
          ...
          <div className="mt-auto shrink-0 space-y-1 border-t border-sidebar-border/70 bg-sidebar px-3 py-3">
            ...
            <form action="/auth/signout" method="post">
              <button
                type="submit"
                className="ml-2 mt-1 text-[11px] font-medium text-[color:var(--sidebar-muted)] transition-colors hover:text-sidebar-foreground"
              >
                Log out
              </button>
            </form>
        ...
        <div className="min-h-screen min-w-0 flex-1">
          <header className="no-print print:hidden sticky top-0 z-30 border-b border-sidebar-border/70 bg-sidebar px-3 py-2 text-sidebar-foreground shadow-[0_1px_8px_rgba(0,0,0,0.08)] lg:hidden">
              ...
              {bottomNavItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    className={cn(
                      'flex min-h-[38px] items-center gap-2.5 rounded-lg px-2.5 py-2 text-sm font-medium',
                      navLinkClass(active)
                    )}
                    aria-current={active ? 'page' : undefined}
                    onClick={() => setMobileMenuOpen(false)}
