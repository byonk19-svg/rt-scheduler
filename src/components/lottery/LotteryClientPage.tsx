...
import { AlertCircle, ArrowDown, ArrowUp, CalendarDays, ClipboardList, Users } from 'lucide-react'
...
function recommendationStateVariant(
  state: LotteryPageSnapshot['recommendation'] extends infer T
...
export default function LotteryClientPage({
  ...
  const [selectedShift, setSelectedShift] = useState<LotteryShiftType>(
    initialSnapshot.selectedShift
  )
  ...
  const [overrideSelections, setOverrideSelections] = useState<
    Record<string, 'working' | 'cancelled' | 'on_call'>
  >({})
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const response = await fetch(`/api/lottery/snapshot?${params.toString()}`, {
        cache: 'no-store',
      })
      const payload = (await response.json().catch(() => null)) as {
        snapshot?: LotteryPageSnapshot
        error?: string
      } | null
      if (cancelled) return
  ...
  const overrideValidationError = useMemo(() => {
    ...
    const cancelledCount = overrideActionSummary.filter(
    ...
    if (
      overrideActionSummary.length > 1 &&
      (onCallCount !== 1 || cancelledCount !== overrideActionSummary.length - 1)
    ) {
              ...
              >
                <div>
                  <p className="font-medium text-foreground">
                    ...
                    {request.shiftType === 'night' ? 'Night' : 'Day'}
                  </p>
                  <p className="text-xs text-muted-foreground">
    ...
    <div className="max-w-6xl space-y-5">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Pick the date, shift, and keep-to-work count before reviewing the recommendation.
          </CardDescription>
        </CardHeader>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)_minmax(0,1fr)]">
            ...
            {loading ? (
              ...
              <>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge variant="outline">Keep working: {recommendation.keepToWork}</Badge>
                  <Badge variant="outline">Reduce: {recommendation.reductionsNeeded}</Badge>
                  {recommendation.overrideApplied ? (
                    <Badge variant="outline">Override on file</Badge>
                  ) : null}
                </div>
                ...
                <div className="space-y-2 rounded-lg border border-border/70 bg-muted/15 px-3 py-3 text-sm">
                  ...
                  {recommendation.latestAppliedAt ? (
                    <p className="text-xs text-muted-foreground">
                      ...
                      {recommendation.latestAppliedBy
                        ? ` by ${recommendation.latestAppliedBy}`
                        : ''}
                      .
                    </p>
                  ) : null}
        ...
        <Card className="lg:order-2">
          ...
          <CardContent className="lg:hidden">
            ...
            <div className="mt-4">
              {mobileSection === 'requests' ? renderRequestList() : renderLotteryList()}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="hidden lg:flex lg:order-3">
          <CardHeader>
            ...
            <CardDescription>
              Fixed full-time order for {selectedShift === 'night' ? 'night' : 'day'} shift, with
              the latest counted Lottery date.
            </CardDescription>
          </CardHeader>
              ...
              {historyItems.map((item) => (
                <div key={item.id} className="rounded-lg border border-border/70 px-3 py-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {item.requestRestored ? (
                      <Badge variant="outline">Request restored</Badge>
                    ) : null}
                  </div>
                  ...
                  {item.invalidatedAt ? (
                    <p className="text-xs text-muted-foreground">
                      ...
                      {item.invalidatedReason
                        ? ` (${item.invalidatedReason.replace('_', ' ')})`
                        : ''}
                      .
                    </p>
                  ) : null}
                ...
                <select
                  ...
                  onChange={(event) =>
                    setOverrideSelections((current) => ({
                      ...current,
                      [candidate.therapistId]: event.target.value as
                        | 'working'
                        | 'cancelled'
                        | 'on_call',
                    }))
