...
export function TeamwiseMark({ size = 'default', className = '' }: TeamwiseLogoProps) {
  ...
  return (
    <svg width={pixelSize} height={pixelSize} viewBox="0 0 32 32" fill="none" className={className}>
      <rect x="4" y="6" width="24" height="22" rx="4" fill="#0F172A" />
      <rect x="4" y="6" width="24" height="6" rx="4" fill="#0F172A" />
      <rect x="6" y="8" width="20" height="2" rx="1" fill="#14B8A6" />
      <path d="M10 18L14 22L22 14" stroke="#14B8A6" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
...
export function TeamwiseLogo({
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`.trim()}>
      ...
      {showWordmark && (
        <span className={`font-semibold tracking-tight text-[#0F172A] ${wordmarkClass(size)}`}>
          Teamwise
        </span>
      )}
    </div>
