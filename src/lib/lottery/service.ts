...
function lotteryServiceError(
  ...
  return {
    ...
    statusMutated: options?.statusMutated,
  }
}

type LotteryListOrderRow = {
  id: string
  display_order: number
}

async function updateLotteryListEntryOrder(args: {
  entryId: string
  ...
  const { data, error } = await admin()
    ...
    .update({
      display_order: args.nextOrder,
      updated_at: new Date().toISOString(),
      updated_by: args.actorUserId,
    })
    .eq('id', args.entryId)
    .eq('display_order', args.expectedOrder)
    .select('id, display_order')
    .maybeSingle()

  if (error) {
    console.error(`Failed to move Lottery list row (${args.stepLabel}):`, error)
    return { ok: false, error: 'Could not reorder the Lottery list.' }
  }

  const row = data as LotteryListOrderRow | null
  if (!row || row.id !== args.entryId || row.display_order !== args.nextOrder) {
    console.error(`Lottery list reorder became stale during ${args.stepLabel}.`, {
...
async function restoreLotteryListSwap(
  args: {
    actorUserId: string
    current: { id: string; originalOrder: number; currentOrder: number }
    target: { id: string; originalOrder: number; currentOrder: number }
  },
  deps?: {
    updateLotteryListEntryOrder?: typeof updateLotteryListEntryOrder
  }
): Promise<boolean> {
  const updateOrder = deps?.updateLotteryListEntryOrder ?? updateLotteryListEntryOrder
  let restored = true

  if (args.target.currentOrder !== args.target.originalOrder) {
    const targetRestore = await updateOrder({
      entryId: args.target.id,
      expectedOrder: args.target.currentOrder,
      nextOrder: args.target.originalOrder,
      actorUserId: args.actorUserId,
      stepLabel: 'rollback-target',
    })
    restored = restored && targetRestore.ok
  }

  if (args.current.currentOrder !== args.current.originalOrder) {
    const currentRestore = await updateOrder({
      entryId: args.current.id,
      expectedOrder: args.current.currentOrder,
      nextOrder: args.current.originalOrder,
      actorUserId: args.actorUserId,
      stepLabel: 'rollback-current',
    })
    restored = restored && currentRestore.ok
  }
...
export async function loadLotteryHistory(args: {
  ...
  if (!canReadLotteryHistory(args.actor, args.therapistId)) {
    return []
  }
...
export async function moveLotteryListEntry(
  args: {
    actor: LotteryActor
    entryId: string
    shiftType: LotteryShiftType
  ...
  deps?: {
    loadLotteryList?: typeof loadLotteryList
    updateLotteryListEntryOrder?: typeof updateLotteryListEntryOrder
    restoreLotteryListSwap?: typeof restoreLotteryListSwap
  }
): Promise<{ ok: true } | { ok: false; error: string }> {
  if (!canManageList(args.actor.role)) {
    return { ok: false, error: 'Only managers can edit the Lottery list.' }
  ...
  const updateOrder = deps?.updateLotteryListEntryOrder ?? updateLotteryListEntryOrder
  const restoreSwap = deps?.restoreLotteryListSwap ?? restoreLotteryListSwap

  const list = await loadList(args.actor.siteId, args.shiftType)
  ...
  reorderState.current.currentOrder = temporaryOrder
  ...
  if (!second.ok) {
    const restored = await restoreSwap(
      {
        actorUserId: args.actor.userId,
        current: reorderState.current,
        target: reorderState.target,
      },
      { updateLotteryListEntryOrder: updateOrder }
    )
    if (!restored) {
      return { ok: false, error: 'Could not reorder the Lottery list safely.' }
    }
    return { ok: false, error: 'Could not reorder the Lottery list.' }
  }
  reorderState.target.currentOrder = currentOrder

  const third = await updateOrder({
    entryId: current.id,
    expectedOrder: temporaryOrder,
  ...
  reorderState.current.currentOrder = targetOrder
...
export async function updateAssignmentStatusWithLottery(args: {
  ...
  | { ok: true; previousStatus: AssignmentStatus }
  | {
      ok: false
      error: string
      code?: string
...
> {
  ...
  if (activeRequestRows.error) {
    return lotteryServiceError(
      'Could not verify active Lottery requests before reconciling the status change.',
      activeRequestRows.error,
      { previousStatus: context.previousStatus, statusMutated: true }
    )
  }

  if (suppressedRequestRows.error) {
    return lotteryServiceError(
      'Could not verify suppressed Lottery requests before reconciling the status change.',
      suppressedRequestRows.error,
      { previousStatus: context.previousStatus, statusMutated: true }
    )
  }
  ...
  if (reconciliation.invalidatePreviousHistory) {
    const { data: currentHistory, error: currentHistoryError } = await admin()
      .from('lottery_history_entries')
      .select('id')
    ...
    if (currentHistoryError) {
      return lotteryServiceError(
        'Could not load the current Lottery history entry to invalidate it.',
        currentHistoryError,
        { previousStatus: context.previousStatus, statusMutated: true }
      )
    }
    ...
    if (!currentHistoryId) {
      return lotteryServiceError(
        'Could not locate the current Lottery history entry to invalidate it.',
        undefined,
        { previousStatus: context.previousStatus, statusMutated: true }
      )
    }
    ...
    const reason =
      args.nextStatus === 'on_call' || args.nextStatus === 'cancelled'
        ? 'status_changed'
        : 'status_reverted'
    const { data: invalidatedHistory, error } = await admin()
      .from('lottery_history_entries')
      .update({
      ...
      .maybeSingle()
    if (error) {
      return lotteryServiceError('Could not invalidate the current Lottery history entry.', error, {
        previousStatus: context.previousStatus,
        statusMutated: true,
      })
    }
    if (!(invalidatedHistory as { id: string } | null)?.id) {
      return lotteryServiceError(
        'Could not invalidate the current Lottery history entry because its state changed.',
        undefined,
        { previousStatus: context.previousStatus, statusMutated: true }
      )
    }
  ...
  if (reconciliation.suppressActiveRequest) {
    const { data: suppressedRequest, error } = await admin()
      .from('lottery_requests')
      .update({
      ...
      .maybeSingle()
    if (error) {
      return lotteryServiceError('Could not suppress the active Lottery request.', error, {
        previousStatus: context.previousStatus,
        statusMutated: true,
      })
    }
    if (!(suppressedRequest as { id: string } | null)?.id) {
      return lotteryServiceError(
        'Could not suppress the active Lottery request because its state changed.',
        undefined,
        { previousStatus: context.previousStatus, statusMutated: true }
      )
    }
  ...
  if (reconciliation.createHistoryEntry) {
    const { data: createdHistory, error } = await admin()
      .from('lottery_history_entries')
      .insert({
      ...
      .maybeSingle()
    if (error) {
      return lotteryServiceError('Could not create the Lottery history entry.', error, {
        previousStatus: context.previousStatus,
        statusMutated: true,
      })
    }
    if (!(createdHistory as { id: string } | null)?.id) {
      return lotteryServiceError(
        'Could not confirm the Lottery history entry was created.',
        undefined,
        { previousStatus: context.previousStatus, statusMutated: true }
      )
    }
  ...
  if (reconciliation.restoreSuppressedRequest) {
    const { data: suppressedRows, error: suppressedRowsError } = await admin()
      .from('lottery_requests')
      .select('id')
      .eq('site_id', context.siteId)
      .eq('therapist_id', context.therapistId)
    ...
    if (suppressedRowsError) {
      return lotteryServiceError(
        'Could not load the suppressed Lottery request that should be restored.',
        suppressedRowsError,
        { previousStatus: context.previousStatus, statusMutated: true }
      )
    }
    ...
    if (!suppressedId) {
      return lotteryServiceError(
        'Could not locate the suppressed Lottery request that should be restored.',
        undefined,
        { previousStatus: context.previousStatus, statusMutated: true }
      )
    }

    const { data: restoredRequest, error } = await admin()
      .from('lottery_requests')
      .update({
        state: 'active',
      ...
      .maybeSingle()
    if (error) {
      return lotteryServiceError('Could not restore the suppressed Lottery request.', error, {
        previousStatus: context.previousStatus,
        statusMutated: true,
      })
    }
    if (!(restoredRequest as { id: string } | null)?.id) {
      return lotteryServiceError(
        'Could not restore the suppressed Lottery request because its state changed.',
        undefined,
        { previousStatus: context.previousStatus, statusMutated: true }
      )
    }

    const { data: invalidatedRows, error: invalidatedRowsError } = await admin()
      .from('lottery_history_entries')
      .select('id')
      .eq('shift_id', context.shiftId)
      .eq('therapist_id', context.therapistId)
    ...
    if (invalidatedRowsError) {
      return lotteryServiceError(
        'Could not load the invalidated Lottery history entry for the restored request.',
        invalidatedRowsError,
        { previousStatus: context.previousStatus, statusMutated: true }
      )
    }
    ...
    if (!invalidatedId) {
      return lotteryServiceError(
        'Could not locate the invalidated Lottery history entry for the restored request.',
        undefined,
        { previousStatus: context.previousStatus, statusMutated: true }
      )
    }
    ...
    if (historyUpdate.error) {
      return lotteryServiceError(
        'Could not mark the restored Lottery request on its history entry.',
        historyUpdate.error,
        { previousStatus: context.previousStatus, statusMutated: true }
      )
    }
    if (!(historyUpdate.data as { id: string } | null)?.id) {
      return lotteryServiceError(
        'Could not mark the restored Lottery request on its history entry because its state changed.',
        undefined,
        { previousStatus: context.previousStatus, statusMutated: true }
      )
    }
...
async function rollbackLotteryStatusChanges(
  args: {
    authClient: SupabaseRpcClient
    actor: LotteryActor
    touched: Array<{ shiftId: string; previousStatus: AssignmentStatus }>
  },
  deps?: {
    updateAssignmentStatusWithLottery?: typeof updateAssignmentStatusWithLottery
  }
): Promise<string | null> {
  const updateStatus = deps?.updateAssignmentStatusWithLottery ?? updateAssignmentStatusWithLottery

  for (const prior of args.touched.slice().reverse()) {
    const rollback = await updateStatus({
    ...
    if (!rollback.ok) {
      return `Rollback failed for shift ${prior.shiftId}: ${rollback.error}`
    }
  }
...
async function restoreSupersededLotteryDecisions(decisionIds: string[]): Promise<string | null> {
  ...
  if (error) {
    console.error('Failed to restore superseded Lottery decisions during rollback:', error)
    return 'Could not restore superseded Lottery decisions.'
  }

  return null
}

async function clearLotteryDecisionHistoryLinks(args: {
  siteId: string
  decisionId: string
  shiftIds: string[]
}): Promise<string | null> {
  if (args.shiftIds.length === 0) return null
  ...
  const { error } = await admin()
    ...
    .in('shift_id', args.shiftIds)

  if (error) {
    console.error('Failed to clear Lottery history decision links during rollback:', error)
    return 'Could not clear Lottery history links for the failed decision.'
  }

  return null
}

async function deleteLotteryDecision(decisionId: string): Promise<string | null> {
  const { error } = await admin().from('lottery_decisions').delete().eq('id', decisionId)

  if (error) {
    console.error('Failed to delete Lottery decision during rollback:', error)
    return 'Could not delete the failed Lottery decision record.'
  }

  return null
}

function combineLotteryFailure(
  primary: string,
  ...
  return {
    ok: false,
    error: `${primary} Cleanup issues: ${cleanupErrors.join(' ')}`,
  }
}

export async function applyLotteryDecision(
  args: {
    actor: LotteryActor
    authClient: SupabaseRpcClient
    shiftDate: string
  ...
  deps?: {
    loadLotterySnapshot?: typeof loadLotterySnapshot
    loadSlotAssignments?: typeof loadSlotAssignments
    updateAssignmentStatusWithLottery?: typeof updateAssignmentStatusWithLottery
    rollbackLotteryStatusChanges?: typeof rollbackLotteryStatusChanges
    admin?: typeof admin
    restoreSupersededLotteryDecisions?: typeof restoreSupersededLotteryDecisions
    clearLotteryDecisionHistoryLinks?: typeof clearLotteryDecisionHistoryLinks
    deleteLotteryDecision?: typeof deleteLotteryDecision
  }
): Promise<{ ok: true } | { ok: false; error: string }> {
  const loadSnapshot = deps?.loadLotterySnapshot ?? loadLotterySnapshot
  const loadAssignments = deps?.loadSlotAssignments ?? loadSlotAssignments
  const updateStatus = deps?.updateAssignmentStatusWithLottery ?? updateAssignmentStatusWithLottery
  const rollbackStatus = deps?.rollbackLotteryStatusChanges ?? rollbackLotteryStatusChanges
  const adminClient = deps?.admin ?? admin
  const restoreSuperseded =
    deps?.restoreSupersededLotteryDecisions ?? restoreSupersededLotteryDecisions
  const clearHistoryLinks =
    deps?.clearLotteryDecisionHistoryLinks ?? clearLotteryDecisionHistoryLinks
  ...
  const shiftAssignments = await loadAssignments({
    siteId: args.actor.siteId,
    shiftDate: args.shiftDate,
    shiftType: args.shiftType,
  })
  ...
  for (const assignment of targetAssignments) {
    ...
    if (!result.ok) {
      const rollbackTargets = touched.slice()
      if (result.statusMutated && result.previousStatus) {
        rollbackTargets.push({
          shiftId: assignment.shiftId,
          previousStatus: result.previousStatus,
        })
      }
      const rollbackError = await rollbackStatus(
        {
          authClient: args.authClient,
          actor: args.actor,
          touched: rollbackTargets,
        },
        { updateAssignmentStatusWithLottery: updateStatus }
      )
      return combineLotteryFailure(result.error, rollbackError ? [rollbackError] : [])
    }

    touched.push({ shiftId: assignment.shiftId, previousStatus: result.previousStatus })
    touchedShiftIds.push(assignment.shiftId)
  }

  const supersedeResult = await adminClient()
    .from('lottery_decisions')
    .update({
  ...
  if (supersedeResult.error) {
    ...
    return combineLotteryFailure(
      'Could not supersede the previous Lottery decision.',
      rollbackError ? [rollbackError] : []
    )
  }
  const supersededDecisionIds = ((supersedeResult.data ?? []) as Array<{ id: string }>).map(
    (row) => row.id
  )
  ...
  const decisionInsert = await adminClient()
    .from('lottery_decisions')
    .insert({
      site_id: args.actor.siteId,
      shift_date: args.shiftDate,
  ...
  if (decisionInsert.error || !decisionInsert.data) {
    ...
    const restoreSupersededError = await restoreSuperseded(supersededDecisionIds)
    if (restoreSupersededError) cleanupErrors.push(restoreSupersededError)
    const rollbackError = await rollbackStatus(
      {
        authClient: args.authClient,
        actor: args.actor,
    ...
    return combineLotteryFailure('Could not save the applied Lottery result.', cleanupErrors)
  }

  const decisionId = (decisionInsert.data as { id: string }).id
  const historyUpdate = await adminClient()
    .from('lottery_history_entries')
    .update({
    ...
    .in('shift_id', touchedShiftIds)
    .is('decision_id', null)
    .select('id')

  if (historyUpdate.error) {
    const cleanupErrors: string[] = []
    const clearLinksError = await clearHistoryLinks({
      siteId: args.actor.siteId,
      decisionId,
      shiftIds: touchedShiftIds,
    })
    if (clearLinksError) cleanupErrors.push(clearLinksError)
    const deleteDecisionError = await deleteDecision(decisionId)
    if (deleteDecisionError) cleanupErrors.push(deleteDecisionError)
    const restoreSupersededError = await restoreSuperseded(supersededDecisionIds)
    if (restoreSupersededError) cleanupErrors.push(restoreSupersededError)
    const rollbackError = await rollbackStatus(
      {
    ...
    if (rollbackError) cleanupErrors.push(rollbackError)
    return combineLotteryFailure(
      'Could not attach Lottery history entries to the new decision.',
      cleanupErrors
    )
  }

  const linkedHistoryRows = ((historyUpdate.data ?? []) as Array<{ id: string }>).length
  if (linkedHistoryRows !== touchedShiftIds.length) {
    const cleanupErrors: string[] = []
    const clearLinksError = await clearHistoryLinks({
      siteId: args.actor.siteId,
      decisionId,
    ...
    if (rollbackError) cleanupErrors.push(rollbackError)
    return combineLotteryFailure(
      'Could not link every Lottery history entry to the new decision.',
      cleanupErrors
    )
  }
