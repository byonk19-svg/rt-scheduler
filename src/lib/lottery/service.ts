...
function resolveLiveAssignmentStatus(rowStatus: string, activeCode: AssignmentStatus | undefined): AssignmentStatus {
...
function buildContextSignature(args: {
  ...
  return JSON.stringify({
    ...
    requestList: args.requestList
      ...
      .sort((a, b) => a.requestedAt.localeCompare(b.requestedAt) || a.therapistId.localeCompare(b.therapistId)),
...
async function loadLotteryList(siteId: string, shiftType: LotteryShiftType): Promise<LotteryListItem[]> {
  const { data, error } = await admin()
    .from('lottery_list_entries')
    .select('id, therapist_id, display_order, profiles:profiles!lottery_list_entries_therapist_id_fkey(full_name)')
    .eq('site_id', siteId)
...
async function loadLastLotteriedDates(args: {
  ...
  const result = new Map<string, string | null>(args.therapistIds.map((therapistId) => [therapistId, null]))
  ...
  const rows = (data ?? []) as Array<{ therapist_id: string; shift_date: string; shift_type: LotteryShiftType }>
...
async function suppressInvalidRequestsForSlot(args: {
  ...
  const toSuppress = requests.filter((request) => !eligibleIds.has(request.therapistId)).map((request) => request.id)
...
async function suppressInvalidRequestsForUser(args: {
  actor: LotteryActor
}): Promise<void> {
  const today = new Date().toISOString().slice(0, 10)
  const { data, error } = await admin()
  ...
  const rows = (data ?? []) as Array<{ id: string; shift_date: string; shift_type: LotteryShiftType }>
  for (const row of rows) {
...
function buildEligibleCandidates(args: {
  ...
  const lotteryListByTherapistId = new Map(args.lotteryList.map((entry) => [entry.therapistId, entry]))
...
export async function loadLotterySnapshot(args: {
  ...
  const [workingAssignmentsAll, lotteryListBase, requestList, myRequests, latestAppliedDecision] =
    await Promise.all([
      loadSlotAssignments({ siteId: args.actor.siteId, shiftDate: selectedDate, shiftType: selectedShift }),
      loadLotteryList(args.actor.siteId, selectedShift),
      loadActiveRequests({ siteId: args.actor.siteId, shiftDate: selectedDate, shiftType: selectedShift }),
      loadMyRequests(args.actor),
      loadLatestAppliedDecision({ siteId: args.actor.siteId, shiftDate: selectedDate, shiftType: selectedShift }),
    ])

  const workingAssignments = workingAssignmentsAll.filter((assignment) => assignment.liveStatus === 'scheduled')
  ...
  const currentUserHasRequest = requestList.some((request) => request.therapistId === args.actor.userId)
  ...
  const requestableTherapists = workingAssignments
    ...
    .filter((assignment) => !requestList.some((request) => request.therapistId === assignment.therapistId))
  ...
  if (args.keepToWork != null) {
    if (!Number.isInteger(args.keepToWork) || args.keepToWork < 0 || args.keepToWork > workingAssignments.length) {
...
export async function addLotteryRequest(args: {
  ...
  if (!slotAssignment || slotAssignment.liveStatus !== 'scheduled') {
    return { ok: false, error: 'Requests are only allowed for therapists currently scheduled on that shift.' }
  }
...
export async function addLotteryListEntry(args: {
  ...
  if (profileResult.error || !profileResult.data) {
    if (profileResult.error) console.error('Failed to load Lottery list candidate:', profileResult.error)
...
export async function moveLotteryListEntry(args: {
  ...
  const second = await admin()
    .from('lottery_list_entries')
    .update({ display_order: currentOrder, updated_at: new Date().toISOString(), updated_by: args.actor.userId })
    .eq('id', target.id)
  if (second.error) {
  ...
  const third = await admin()
    .from('lottery_list_entries')
    .update({ display_order: targetOrder, updated_at: new Date().toISOString(), updated_by: args.actor.userId })
    .eq('id', current.id)
  if (third.error) {
...
async function loadShiftStatusMutationContext(args: {
  shiftId: string
}): Promise<{
  shiftId: string
...
> {
  ...
  if (reconciliation.invalidatePreviousHistory) {
    ...
    if (currentHistoryId) {
      const reason = args.nextStatus === 'on_call' || args.nextStatus === 'cancelled' ? 'status_changed' : 'status_reverted'
      const { error } = await admin()
        .from('lottery_history_entries')
        .update({
  ...
  if (reconciliation.createHistoryEntry) {
    const { error } = await admin().from('lottery_history_entries').insert({
      ...
      if (error) {
        ...
        if (invalidatedId) {
          ...
          if (historyUpdate.error) {
            console.error('Failed to mark Lottery request restoration on history entry:', historyUpdate.error)
          }
        }
      }
    }
...
export async function applyLotteryDecision(args: {
  ...
  if (!snapshot.recommendation) {
    return { ok: false, error: snapshot.recommendationError ?? 'A recommendation is required before applying.' }
  }
  ...
  const overrideApplied = stringifyActions(
    args.actions.map((action) => ({
      therapistId: action.therapistId,
      therapistName:
        appliedActions.find((applied) => applied.therapistId === action.therapistId)?.therapistName ??
        action.therapistId,
      status: action.status,
    }))
  ...
  if (args.actions.length > 1 && (onCallCount !== 1 || cancelledCount !== args.actions.length - 1)) {
    return {
      ok: false,
      error: 'Multi-person reductions must have exactly one on-call therapist and cancel everyone else.',
    }
  }
  ...
  const eligibleAssignments = shiftAssignments.filter((assignment) => assignment.liveStatus === 'scheduled')
  const targetAssignments = eligibleAssignments.filter((assignment) => actionSet.has(assignment.therapistId))

  if (targetAssignments.length !== args.actions.length) {
    return { ok: false, error: 'One or more selected therapists are no longer eligible for this Lottery slot.' }
  }
  ...
  const appliedActionsView = args.actions
    ...
    .sort((a, b) => a.status.localeCompare(b.status) || a.therapistName.localeCompare(b.therapistName))
  ...
  const historyUpdate = await admin()
    ...
    .in('shift_id', touched.map((item) => item.shiftId))
    .is('decision_id', null)
