...
describe('schedule lifecycle hardening', () => {
  ...
  it('keeps offline and archived Schedule Blocks read-only outside lifecycle actions', () => {
    ...
    expect(shiftActionsSource).toContain('isPublishedScheduleBlock')
    expect(templateActionsSource).toContain("cycle.status !== 'draft'")
    expect(dragDropRouteSource).toContain('loadScheduleMutationCycle')
    expect(dragDropRouteSource).toContain('isPreliminaryScheduleBlock')
    expect(loadScheduleMutationCycleSource).toContain('isReadOnlyScheduleBlock(cycle)')
    expect(assignmentStatusRouteSource).toContain('isReadOnlyScheduleBlock(preflightCycle)')
  })
})
